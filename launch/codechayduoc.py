    ...
    def timer_callback(self):
            ...
            if self.detections:
                ...
                for i, det in enumerate(self.detections):
                    ...
                    pred_x, pred_y, _, _ = kf.predict()  # Sử dụng tọa độ đã làm mượt

                    # Ghi log thông tin sau khi làm mượt bằng Kalman Filter
                    self.get_logger().info(f"Phát hiện '{det['label']}' at position (x={pred_x:.2f}, y={pred_y:.2f})")

                    # Xuất bản marker với tọa độ đã làm mượt
